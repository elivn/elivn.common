...
namespace Kasca.OrmPlug.SqlServer
{
    ...
    /// 仓储层基类
    /// </summary>
    public class BaseSqlServerRep<TRep, TType>
        ...
        //where TType:BaseMo,new()
    {
        ...
        static BaseSqlServerRep()
        {
            ...
            SqlVistorFlag.SetDbProvider(SqlVistorProvider.SqlServer);
        }
            ...
            where RType : ResultMo, new()
        {
            ...
            try
            {
                using (var con = new System.Data.SqlClient.SqlConnection(connecStr))
                {
                    t = await func(con);
                }
            }
        ...
        /// <param name="mo"></param>
        /// <returns></returns>
        public virtual async Task<ResultMo> Add(TType mo, bool isIdMainKey = true)
            => await ExcuteWriteAsync(con => con.Insert(m_TableName, mo,isIdMainKey));
        ...
        public virtual async Task<ResultMo> AddList(IList<TType> list)
        {
            var res = await ExcuteWriteAsync(async con =>
            {
                ...
                return row > 0 ? new ResultMo() : new ResultMo(ResultTypes.AddFail, "添加失败!");
            });
            return res;
        }
        ...
        /// 批量插入功能 bulkCopy
        /// </summary>
        public virtual async Task<ResultMo> AddBatch<RType>(IEnumerable<RType> entityList) where RType : class
        {

            ResultMo t;
            try
            {
                using (var bulkCopy = new SqlBulkCopy(m_writeConnectionString, SqlBulkCopyOptions.TableLock))
                {
                    ...
                    var classMap = sqlGenerator.Configuration.GetMap<RType>();
            ...
            {
                LogUtil.Error(string.Concat($"数据库操作错误，仓储表：({m_TableName})详情：", e.Message, "\r\n", e.StackTrace), "AddBatch", "DapperRep");
                t = new ResultMo
                {
                    code = (int)ResultTypes.InnerError,
                    msg = "数据更新出错！"
            ...
            return t ?? new ResultMo() { code = (int)ResultTypes.ObjectNull, msg = "未发现对应结果" };
        ...
        public virtual async Task<ResultMo<TType>> GetById(string id)
        {
            ...
            var sql = string.Concat("select * from ", m_TableName, $" WHERE \"{m_TableName.TrimStart('t').TrimStart('x').TrimStart('_')}Id\"=@id");
        ...
        ///  查询数据条数
        /// </summary>
        /// <param name="whereExp">判断条件</param>
        /// <returns></returns>
        protected virtual async Task<ResultMo<int>> Count(Expression<Func<TType, bool>> whereExp)
            => await ExecuteReadeAsync(con => con.Count(m_TableName, whereExp));
        ...
        /// <param name="whereExp">判断条件</param>
        /// <returns></returns>
        protected virtual async Task<ResultMo<bool>> Exists(Expression<Func<TType, bool>> whereExp)
            => await ExecuteReadeAsync(async con => (await con.Count(m_TableName, whereExp) > 0));
