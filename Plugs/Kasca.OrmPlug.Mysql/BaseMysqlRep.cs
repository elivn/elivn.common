...
namespace Kasca.OrmPlug.Mysql
{
        ...
        //where TType:BaseMo,new()
    {
        ...
        /// <param name="func"></param>
        /// <returns></returns>
        protected internal static async Task<ResultMo<RType>> ExecuteReadeAsync<RType>(Func<IDbConnection, Task<RType>> func) =>await Execute(async con =>
        {
        ...
        public virtual async Task<ResultMo> AddBatch<RType>(IEnumerable<RType> entityList) where RType : class
        {
            ...
            try
            {
                t = await AddList(entityList as IList<TType>);
            }
            catch (Exception e)
            {
        ...
        protected internal static async Task<ResultMo<IList<TType>>> GetList(Expression<Func<TType, bool>> whereExp)
            => await ExecuteReadeAsync(con => con.GetList(m_TableName, whereExp));
        ...
        public virtual async Task<ResultMo<TType>> GetById(long id)
        {
        ...
        {
            return await ExecuteReadeAsync(con => con.QuerySingleOrDefaultAsync<TType>(sql, para));
        }
